...
<x-app-layout>
    ...
    <div class="grid grid-flow-col grid-cols-4 gap-4 ">
        <div class="row-span-4 text-sm font-bold leading-6 text-center text-gray-500 rounded-lg bg-stripes-fuchsia">
        ...
        <div class="grid grid-flow-col grid-cols-2 col-span-3 row-span-2 gap-4 pb-10 ">
            <div class="justify-between col-span-1 space-x-4 row-span-3flex ">
                <div class="col-span-1 bg-white rounded-lg w-2s/5 shadow-card mb-9">
                    <div class="flex justify-between px-5 py-5 space-x-2 border-b">
                        ...
                        <div class="flow-root mt-6">
                            <ul role="list" class="px-0 pl-0 -my-5 divide-y divide-gray-200">
                                @forelse ($faqs as $faq)
                                <li class="mb-2 ">
                                    <div class="relative px-5 py-4 text-gray-500 hover:text-blauw-hover">
                                        <h3 class="text-sm font-semibold text-gray-800 hover:text-blauw-hover">
                                            <a href="{{ route('faq.show', $faq->id ) }}" class="hover:underline focus:outline-none ">
                                                <!-- Extend touch target to entire panel -->
                                                <span class="absolute inset-0" aria-hidden="true"></span>
                                                {{ $faq->title }}
                                            </a>
                                        </h3>
                                        <p class="mt-1 text-sm line-clamp-2">{{ strip_tags($faq->description) }}</p>
                                    </div>
                                </li>
                                @empty
                                @endforelse
                ...
                <div class="flex-1 col-span-1 row-span-1 bg-white rounded-lg shadow-card mb-9 ">@livewire('ticket-create-box')
                </div>
            </div>
